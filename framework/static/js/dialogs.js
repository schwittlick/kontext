...
$(function() {
    ...
    function submitMonitorModal(event){
        ...
        switch(type){
            case 'gate':
                ...
                params['gate'] = $('#monitor_'+type+'_input').val()
                break;
            case 'link':
    ...
    function update_fragment(){
        var node_uids = Object.keys(selection);
        if(node_uids.length == 0){
            node_uids = null;
        }
        api.call("generate_recipe_code", {
            'nodenet_uid': currentNodenet,
            'recipe_name': $('#fragment_recipe_name').val(),
            'node_uids': node_uids,
            'with_env_config': $('#fragment_env')[0].checked,
            'with_device_config': $('#fragment_devices')[0].checked,
            'with_state': $('#fragment_state')[0].checked
        }, function(data){
            $("#fragment_code").text(data);
        });
    }
    $('.navbar .create_netapi_fragment').on('click', function(event){
        $('#netapi_fragment').modal('show');
        $('#fragment_recipe_name').val($('#agent_name').text().trim())
        $('#netapi_fragment button').prop('disabled', false);
        update_fragment();
    });
    $('#netapi_fragment_form .refresh_code').on('change', function(event){
        update_fragment();
    });
    $('#fragment_recipe_name').on('change', function(event){
        if($('#fragment_recipe_name').val()){
            $('#fragment_recipe_save').removeAttr("disabled");
        } else{
            $('#fragment_recipe_save').attr("disabled", "disabled");
        }
    });
    $('#fragment_recipe_save').on('click', function(event){
        event.preventDefault();
        var node_uids = Object.keys(selection);
        if(node_uids.length == 0){
            node_uids = null;
        }
        api.call("save_recipe", {
            'nodenet_uid': currentNodenet,
            'recipe_name': $('#fragment_recipe_name').val(),
            'node_uids': node_uids,
            'file_name': $('#fragment_recipe_file_name').val(),
            'with_env_config': $('#fragment_env')[0].checked,
            'with_device_config': $('#fragment_devices')[0].checked,
            'with_state': $('#fragment_state')[0].checked
        }, function(data){
            dialogs.notification("Recipe saved");
            $('#netapi_fragment').modal('hide');
        });
    });
});
...
$(document).ready(function() {
    api.call("get_loaded_nodenet_uids", {}, function(active_uids){
        if(active_uids.length){
            currentNodenet = active_uids[0];
        } else {
