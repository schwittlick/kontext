...
function setNodespaceData(data, changed){
    ...
    if (data && !jQuery.isEmptyObject(data)){
        ...
        for(uid in data.nodes){
            ...
            item = new Node(uid, node['position'][0], node['position'][1], node.parent_nodespace, node.name, node.type, node.activation, node.state, node.parameters, node.gate_activations, node.gate_configuration, node.inlinks, node.outlinks, node.inputmap);
            if(uid in nodes){
    ...
    if (data && !jQuery.isEmptyObject(data)){
        ...
        if(data.changes){
            ...
            for(var uid in data.changes.nodes_dirty){
                ...
                item = new Node(uid, nodedata['position'][0] + pos_correction.x, nodedata['position'][1] + pos_correction.y, nodedata.parent_nodespace, nodedata.name, nodedata.type, nodedata.activation, nodedata.state, nodedata.parameters, nodedata.gate_activations, nodedata.gate_configuration, nodedata.inlinks, nodedata.outlinks);
                if(uid in nodes){
...
function Node(uid, x, y, nodeSpaceUid, name, type, activation, state, parameters, gate_activations, gate_configuration, inlinks, outlinks, inputmap) {
	this.uid = uid;
	this.x = x;
	this.y = y;
    ...
    for(i in nodetypes[type].gatetypes){
        ...
        this.gates[gatetype] = new Gate(gatetype, i, activation, this.gate_configuration[gatetype]);
    }
...
function Gate(name, index, activation, gate_configuration) {
	this.name = name;
    this.index = index;
	this.outgoing = {};
...
function openContextMenu(menu_id, event) {
    ...
    if(menu_id == '#create_node_menu'){
        ...
        if(Object.keys(flow_modules).length){
            flow_html = '';
            flow_html += '<li class="divider"></li><li class="noop"><a>Create Flow Module<i class="icon-chevron-right"></i></a>';
            flow_html += '<ul class="sub-menu dropdown-menu">';
            add_flowmenu = false;
            flow_html += buildRecursiveDropdown(flow_module_categories, '', '', function(current_category){
                items = '';
                for(var idx in sorted_flow_modules){
                    key = sorted_flow_modules[idx];
                    if(flow_modules[key].category == current_category && !flow_modules[key].is_autogenerated){
...
function deleteNodeHandler(nodeUid) {
    ...
    if(deletedNodes.length){
        api.call('delete_nodes', {nodenet_uid: currentNodenet, node_uids: deletedNodes}, function(){
            ...
            dialogs.notification(result.data, 'error');
        ...
        if ((sourceUid in nodes)) {
            if(!(targetUid in nodes)){
                api.call('get_node', {
                    ...
                    nodes[targetUid] = new Node(data.uid, data.position[0], data.position[1], data.parent_nodespace, data.name, data.type, data.activation, data.state, data.parameters, data.gate_activations, data.gate_configuration, data.inlinks, data.outlinks);
...
function showNodeForm(nodeUid, refresh){
    if(refresh){
        api.call('get_node', {
            ...
            item = new Node(nodeUid, data['position'][0], data['position'][1], data.parent_nodespace, data.name, data.type, data.activation, data.state, data.parameters, data.gate_activations, data.gate_configuration, data.inlinks, data.outlinks);
            redrawNode(item);
